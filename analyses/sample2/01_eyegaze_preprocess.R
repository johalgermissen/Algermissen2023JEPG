...
allbehavData <- lapply(list.files(behavDataDir,pattern=".csv",full=TRUE), read.csv, header=T)
behavData <- do.call("rbind",allbehavData)

## Subject 34 accidentally named 33 within file:
# Identify via age:
...
for (iSub in 1:nSub){ # iSub <- 1
  ...
  ## Read values for behavioral variables from eye-tracking data:
  ...
  trialData <- applyInterpolation(trialData, variable="xp", maxGap = maxGap, maxKeepTrial = maxKeepTrial, padEdges = F, interactive = F) # x
  trialData <- applyInterpolation(trialData, variable="yp", maxGap = maxGap, maxKeepTrial = maxKeepTrial, padEdges = F, interactive = F) # y
